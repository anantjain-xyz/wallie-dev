...
3. **Authorize this extension separately.** The existing temporary Supabase network grant excludes endpoint and route changes, and the host grant is for the host foundation. Render and review the [expiring image-pull network grant](AWS-POSTGRES-IMAGE-PULL-GRANT.md) before a future apply. `wallie-local` already has 10/10 managed-policy attachments; inventory every attachment and restore the temporarily swapped policy after the approved operation.
