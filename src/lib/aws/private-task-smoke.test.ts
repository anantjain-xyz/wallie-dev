    ...
    ["web", "worker"].map((component) => {
      ...
      return [
        component,
        {
          ...
          taskDefinition: {
            ...definition,
            taskDefinitionArn: `arn:aws:ecs:${region}:${account}:task-definition/wallie-staging-${component}-connectivity-smoke:1`,
            revision: 1,
            status: "ACTIVE",
            requiresAttributes: [],
...
function evidence(plan = fixture(), component = "web", index = 0) {
  ...
  const logRequest = {
    ...
    logStreamName: `wallie-smoke-${runId}/smoke/${taskId}`,
    startFromHead: true,
  };
  return {
    definitions,
