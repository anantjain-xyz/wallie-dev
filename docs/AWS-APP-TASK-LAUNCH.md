...
2. Render and review one registration policy. Use an explicit UTC expiry 5 minutes–24 hours ahead. The run-phase expiry must cover observation **and stop/cleanup**; an administrator must be ready to stop the exact recorded task if the grant expires early. Registration permits `RegisterTaskDefinition` only for `wallie-staging-web-app:*` and `wallie-staging-worker-app:*`, 512 CPU/1024 MiB, Fargate compatibility, fixed tags, and `PassRole` only for the two existing execution roles. It grants no `RunTask`.
