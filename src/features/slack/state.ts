...
export function createSlackInstallState(
  payload: Omit<SlackInstallState, "createdAt" | "version">,
...
export function verifySlackInstallState(
  ...
  let parsed: SlackInstallState;

  try {
    parsed = JSON.parse(decodeBase64Url(encodedPayload)) as SlackInstallState;
  } catch {
    return null;
  }

  if (parsed.version !== slackStateVersion) {
    return null;
  }
