...
Run `pnpm test:benchmark:interaction` against the fixed local Supabase seed to record request counts, completed-request transferred bytes, click-to-visible-state for Pipeline → Sessions and Sessions → Detail, and the zero-idle-detail-prefetch assertion. The detail measurement waits for the selected session’s title and a visible stage control, rather than just a URL change or loading placeholder. Start with a fresh `supabase db reset` seed; `supabase/tests/seed_session_stages.sql` verifies that all 18 demo sessions expose their four selected stages through the detail endpoint. Network counts cover requests started between the click and 250 ms after content becomes visible. The report separates completed, failed, and still-pending requests; `completedTransferredBytes` includes only requests finished within that window, so open RSC/prefetch streams cannot stall collection. This byte metric is not directly comparable to the previous unbounded total. The benchmark reports timings but deliberately has no millisecond threshold.
