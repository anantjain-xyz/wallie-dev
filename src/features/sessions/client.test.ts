...
describe("updateSessionTitleFromClient", () => {
  ...
  it("rejects an empty title before calling the API", async () => {
    const fetchMock = mockFetch({ body: { title: "Updated" }, ok: true });

    await expect(
      updateSessionTitleFromClient({ sessionId: SESSION_ID, title: "   " }),
    ).rejects.toThrow("Title is required.");

    expect(fetchMock).not.toHaveBeenCalled();
  });

  it("patches the normalized session title and returns the updated title", async () => {
  ...
  it("surfaces update API errors", async () => {
    ...
    await expect(
      updateSessionTitleFromClient({ sessionId: SESSION_ID, title: "Updated title" }),
    ).rejects.toThrow("Session not found.");
  });

  it("rejects malformed update success responses", async () => {
    mockFetch({ body: { id: SESSION_ID, title: "Updated title" }, ok: true });

    await expect(
      updateSessionTitleFromClient({ sessionId: SESSION_ID, title: "Updated title" }),
    ).rejects.toThrow("Session title response was malformed.");
  });
});
