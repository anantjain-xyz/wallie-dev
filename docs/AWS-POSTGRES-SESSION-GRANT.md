...
| `WallieStagingPostgresSessionHost`    | Full host-plan EC2 and exact instance-profile reads; one named CloudWatch log group and 90-day retention; an inline policy on the exact bounded host role.                       |
...
The account-wide Session Manager logging preferences and [exact-instance operator `StartSession` grant](AWS-POSTGRES-OPERATOR-SESSION.md) remain separate gates. Verify the private transcript stream with a no-secrets shell and CloudTrail start/end events before database administration. [Session logging activation](AWS-POSTGRES-SESSION-LOGGING.md)

The host policy repeats the former deployment grant's regional EC2 inventory reads and adds `iam:GetInstanceProfile` on the one host profile. A full host plan refreshes those resources after the foundation grant has been detached. It grants no EC2 or profile writes.

The pinned AWS provider sends log-group tags and deletion protection in `CreateLogGroup`, then sets 90-day retention with `PutRetentionPolicy`. A prior [live application log-group apply](AWS-APPLICATION-FOUNDATION.md) failed with a bare-ARN creation-tag grant, so this grant uses the exact group's `:*` ARN for creation and retention, both bare and `:*` forms for creation tagging, and the bare ARN for tag reads. [Provider source](https://raw.githubusercontent.com/hashicorp/terraform-provider-aws/v6.65.0/internal/service/logs/group.go), [CloudWatch Logs IAM actions](https://docs.aws.amazon.com/service-authorization/latest/reference/list_logs.html), [EC2 IAM actions](https://docs.aws.amazon.com/service-authorization/latest/reference/list_ec2.html)
