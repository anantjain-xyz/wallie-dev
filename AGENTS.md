...
- **Workspace secrets live in the DB, not env.** Linear keys, agent credentials, and sandbox provider connections are entered in Settings and stored AES-256-GCM encrypted (`src/lib/secrets/crypto.ts`).
...
- **Prompt trust boundaries are typed.** The production `runStage()` path classifies control text with `trustedPromptValue()` and session/feedback/artifact data with `untrustedPromptValue()` before passing them to `renderStagePrompt()`. When extending prompts, add an explicit source classification, preserve the typed value through the rendering API, and cross into a renderer string only with `verifyPromptBoundary()`; never pass a raw string across the prompt boundary.
- **Concurrency uses separate guards.** Queue claims, session generation claims, artifact publication, and stage approvals each enforce their own expected state. Rejection only CAS-claims its initial count increment and still has the approval race documented in `docs/PIPELINE-WORKER-LIFECYCLE.md`; preserve and test every boundary when changing transitions.
