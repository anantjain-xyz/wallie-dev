...
const usage =
  "Usage: node scripts/prepare-aws-postgres-deployment.mjs --policy host-boundary --account-id <12 digits> --region <commercial AWS region> OR --policy identity-network|compute-storage --account-id <12 digits> --region <commercial AWS region> --vpc-id <vpc-id> --subnet-id <database-a-subnet-id> --database-security-group-id <db-sg-id> --ami-id <amazon-al2023-ami-id> --kms-key-arn <reviewed-customer-managed-ebs-key-arn> --expires-at <UTC timestamp>";
...
try {
  ...
  const boundaryMode = selectedPolicy === "host-boundary";
  const expectedOptions = boundaryMode ? 3 : 9;
  if (
    positionals.length !== 0 ||
    tokens.filter((token) => token.kind === "option").length !== expectedOptions ||
    Object.keys(values).length !== expectedOptions ||
    Object.values(values).some((value) => value !== value.trim() || value === "") ||
    !["host-boundary", "identity-network", "compute-storage"].includes(selectedPolicy) ||
    !/^[0-9]{12}$/.test(account ?? "") ||
  ...
  const template = readFileSync(
    new URL(
      boundaryMode
        ? "../infra/aws/postgres-host-boundary-policy.template.json"
        : "../infra/aws/postgres-deployment-policy.template.json",
      import.meta.url,
    ),
    "utf8",
  );
  ...
  if (!boundaryMode) {
    policy.Statement = policy.Statement.filter(
      (statement) =>
        computeStorageSids.has(statement.Sid) === (selectedPolicy === "compute-storage"),
    );
