...
- `sessions-status-filter` switches Active / All / Archived via the Session scope control (option names include facet counts, e.g. `All 5`).
...
- **Status filter.** Open the Session scope control and choose All. Run `node .cursor/skills/verify-wallie/scripts/control-wallie.mjs browser click --role combobox --name "Session scope"` then `... browser click --role option --name "/^All /" --wait-for-url "scope=all"`. Option accessible names are `Active N` / `All N` / `Archived N` (facet counts); the closed trigger still shows the bare label. Wait until the URL includes `scope=all` before the next command. The ledger refreshes without error and still shows the Sessions heading.
...
- Scope is a combobox (`Session scope`), not separate Active/All/Archived buttons. Prefer `/^All /` (or `/^Active /` / `/^Archived /`) over bare `"All"` so the option with the count is targeted, not a substring collision.
