...
Transfer `scripts/check-aws-postgres-mount.sh` beside the installed preparation helper using the same reviewed, root-owned transfer and SHA-256 comparison. After the volume is mounted, run the read-only guard with the **independently recorded** values:
