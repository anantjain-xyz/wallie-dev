...
- `readback-matches-manifest` means **offline metadata matched**. It does not establish effective IAM or live task capability. Private image pulls, log delivery, deployment `PassRole`, application task roles, and per-component secret access remain later batches.
