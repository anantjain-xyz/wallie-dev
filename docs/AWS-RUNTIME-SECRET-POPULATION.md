...
**Write the first real web and worker versions only for an isolated staging Supabase database.** Run this locally after reviewing and merging the helper. It does not create a database, grant IAM access, or start tasks.

| Gate         | Requirement                                                                                                                                                                                                            |
| ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Database     | Fresh schema-only Supabase project or branch; apply migrations without seeds or copied application rows. Require no active integrations or runnable Wallie jobs. Never point the worker at the current Wallie project. |
| Inputs       | Staging `sb_secret_…` key and one **new** 64-character random hex `WALLIE_ENCRYPTION_KEY`, kept in a password manager. Do not reuse the production encryption key.                                                     |
...
- Use the existing `WallieStagingRuntimeSecrets` managed policy **version** for the temporary grant after an administrator checks its current versions and every attached identity; `wallie-local` already occupies all ten managed-policy attachment slots. Preserve the original default version for rollback. This helper never changes IAM.
...
- At the hidden prompts, paste the isolated Supabase secret key and the **same fresh staging encryption key** twice. The script does not read app values from the environment or disk.
...
- After both versions match, revoke the temporary write grant. Feed the two IDs into [the runtime configuration map](AWS-RUNTIME-CONFIG.md) and later real task definitions; those must pin their own component version. The worker stays stopped until the isolated database and HTTPS egress are verified.
