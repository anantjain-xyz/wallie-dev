...
| Check   | Evidence                                                                                                                                                                                                                                                                                                                           |
| ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Network | No public ENI address or subnet default route. With image pull off, DB SG has only reviewed API/5432 ingress and SSM/443 egress; a later [image path](AWS-POSTGRES-IMAGE-PULL.md) adds narrow ECR/S3 egress. SSM endpoint SG has only DB/443 ingress and no egress. Inspect all live rules; standalone rules do not detect extras. |
...
- Add a private session-transcript destination with scoped network/IAM access. Current Logs access excludes this host; the default-off image-layer S3 route does not provide session logging. [AWS Session Manager logging](https://docs.aws.amazon.com/systems-manager/latest/userguide/session-manager-logging.html).
