...
- Never run `pkill`/`killall`/`pgrep -f` with a broad pattern (e.g. `pkill -f "next dev"`, `pkill -f node`). Agents share the host with the operator's dashboard and with other workspaces' dev servers — unscoped matches kill _every_ matching process, including the operator's. Scope cleanup to the port or the workspace: prefer `kill "$(lsof -t -i :<PORT>)"` (or `lsof -t -i :<PORT> | xargs -r kill`); when matching by command line, anchor to this workspace (`pkill -f 'next-server.*'"$ISSUE_IDENTIFIER"`). Same rule for `kill %1` / `kill %<jobspec>` only — those are scoped to the current shell.
