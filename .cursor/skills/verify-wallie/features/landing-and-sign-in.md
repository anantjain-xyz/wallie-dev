...
- **Fill email.** Enter a work address. Run `node .cursor/skills/verify-wallie/scripts/control-wallie.mjs browser fill --role textbox --name "Work email" --value "anant@example.com"`. The field shows that value and `Send magic link` remains available.
