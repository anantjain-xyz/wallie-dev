...
- **Submit feedback.** Fill feedback and queue. Run `node .cursor/skills/verify-wallie/scripts/control-wallie.mjs browser fill --role textbox --name "Feedback for Wallie" --value "Needs clearer acceptance criteria"` then `... browser click --role button --name "Queue rerun"`. The dialog closes and the session shows a rerun / in-progress style state (worker required for full progress).
- **Approve path.** On another awaiting-review session, choose the approve button label shown (`Approve & advance` or `Approve & archive`). Run `... browser click --role button --name "Approve & advance"` (or the archive variant). The UI leaves awaiting-review for that artifact.
