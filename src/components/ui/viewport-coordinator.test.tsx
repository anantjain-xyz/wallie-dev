...
describe("ViewportCoordinator", () => {
  it("tracks the visual viewport and scrolls linked validation content above the keyboard", () => {
    ...
    expect(
      document.documentElement.style.getPropertyValue("--wallie-visual-viewport-offset-top"),
    ).toBe("0px");
    expect(
      document.documentElement.style.getPropertyValue("--wallie-visual-viewport-bottom-offset"),
    ).toBe("468px");
    expect(scrollBy).toHaveBeenLastCalledWith({ behavior: "instant", top: 56 });
  });

  it("prioritizes the focused field when its description group cannot fit", () => {
    const viewport = new VisualViewportStub();
    Object.defineProperty(window, "visualViewport", {
      configurable: true,
      value: viewport,
    ...
    const view = render(
      <>
        <ViewportCoordinator />
        <header data-shell-header="" />
        <textarea aria-describedby="long-error" aria-label="Prompt" />
        <p id="long-error" role="alert">
          A distant validation summary.
        </p>
      </>,
    );
    const header = view.container.querySelector("header")!;
    const field = view.getByLabelText("Prompt");
    const error = view.getByRole("alert");
    ...
    expect(scrollBy).toHaveBeenLastCalledWith({ behavior: "instant", top: 60 });
  });
});
