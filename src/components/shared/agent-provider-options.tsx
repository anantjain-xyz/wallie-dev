...
export const AGENT_PROVIDER_SELECT_OPTIONS = AGENT_PROVIDERS.map(
  (provider) =>
    ({
      icon: <AgentProviderLogo provider={provider} />,
...
function ClaudeLogoIcon({ className, style, ...props }: IconProps) {
  return (
    <svg
      aria-hidden="true"
      className={cn("h-4 w-4 shrink-0", className)}
...
function CursorLogoIcon({ className, ...props }: IconProps) {
  return (
    <svg
      ...
      {...props}
    >
      <path d="M5 3v18l5-5 3 5 3-2-3-5h7L5 3Z" fill="currentColor" />
    </svg>
  );
}
