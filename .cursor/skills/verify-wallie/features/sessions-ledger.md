...
- `sessions-status-filter` switches Active / All / Archived via the Session scope radiogroup (radio names include facet counts, e.g. `All 5`).
...
- **Status filter.** Choose All in the Session scope radiogroup. Run `node .cursor/skills/verify-wallie/scripts/control-wallie.mjs browser click --role radio --name "/^All /" --wait-for-url "scope=all"`. Radio accessible names are `Active N` / `Archived N` / `All N` (facet counts on `aria-label`; visible chip text is the bare label plus count). Wait until the URL includes `scope=all` before the next command. The ledger refreshes without error and still shows the Sessions heading.
...
- Scope is a `radiogroup` named `Session scope` with filter-chip `radio` controls — not a combobox/option menu. Prefer `/^All /` (or `/^Active /` / `/^Archived /`) over bare `"All"` so the radio with the count is targeted, not a substring collision.
