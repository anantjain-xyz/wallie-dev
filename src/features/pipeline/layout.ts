...
export const pipelineLayout = {
  page: "flex min-h-[calc(100svh-3.5rem-env(safe-area-inset-top)-env(safe-area-inset-bottom))] flex-col bg-canvas pipeline-wide:h-[calc(100svh-3.5rem-env(safe-area-inset-top)-env(safe-area-inset-bottom))] pipeline-wide:min-h-0 pipeline-wide:lg:h-[calc(100svh-3rem-env(safe-area-inset-top)-env(safe-area-inset-bottom))]",
  controlsArea: "shrink-0 px-4 py-3 pipeline-wide:px-8 pipeline-wide:pb-4 pipeline-wide:pt-10",
  controls:
    "grid grid-cols-1 gap-2 border-0 py-0 min-[380px]:grid-cols-[minmax(0,1fr)_auto] pipeline-wide:mb-5 pipeline-wide:flex pipeline-wide:gap-3 pipeline-wide:border-y pipeline-wide:py-3",
  ...
  laneHeader:
    "z-10 -mx-1 mb-3 border-b border-border/60 bg-canvas/95 px-1 pb-3 backdrop-blur-sm pipeline-wide:sticky pipeline-wide:top-0",
} as const;
