...
- Follow the [data-volume preparation procedure](AWS-POSTGRES-DATA-VOLUME.md): resolve the live EBS ID, initialize only a proven blank volume, mount by filesystem UUID, and persist **both** PGDATA and `/etc/postgresql-custom` (`pgsodium_root.key`). Verify reboot/remount before runtime work.
- Install and run the [read-only mount guard](AWS-POSTGRES-DATA-VOLUME.md#guard-the-future-database-start) with the recorded volume ID and UUID. The future systemd unit must run it before every database start and remain bound to the exact mount; the guard alone does not start PostgreSQL.
- Prepare the separate [empty backup destination](AWS-POSTGRES-BACKUP-DESTINATION.md), then add PostgreSQL 17 base backups, continuous WAL archival, retention, monitoring, and an isolated timed restore. Back up the pgsodium key separately. An online EBS snapshot or `pg_verifybackup` alone does not prove recovery. [PostgreSQL recovery](https://www.postgresql.org/docs/17/continuous-archiving.html), [Supabase key guidance](https://supabase.com/docs/guides/self-hosting/postgres-upgrade-17).
