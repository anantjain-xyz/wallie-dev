...
| Configuration | Encryption, owner controls, all public-access blocks, and the two-deny bucket policy |
...
1. Use a fresh non-root `wallie-staging` login. Verify STS returns account `111614490109`, region `us-west-2`, and user `wallie-local`. Confirm the exact planned bucket name is absent with `head-bucket`; **404** is expected, while **403** is inconclusive. Stop if the name exists or ownership is uncertain. Record the current state bucket, Terraform state keys, and any prior backup root state.
...
3. Pick a UTC deadline 2–24 hours ahead. Render the deployment policy locally under ignored `.wallie/aws/`. Inspect its exact bucket ARN, actions, conditions, and expiry.
...
2. Initialize the separate backend and save a **full, untargeted** plan. Require one bucket plus its five controls: versioning, SSE-S3, owner enforcement, public-access block, and bucket policy. Require **six additions, zero changes, zero deletions**, the exact name/account/region, Object Lock enabled, `force_destroy = false`, and both `DenyInsecureTransport` and `DenyUploadsUntilRecoveryControls` statements. Do not approve a plan with any object, retention rule, lifecycle expiration, IAM, route, or compute resource. Require at least 90 minutes of grant lifetime before apply.
