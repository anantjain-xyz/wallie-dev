...
- The [registry root](AWS-STAGING-REGISTRY.md) reserves a dedicated repository for the [pinned Supabase database image](SELF-HOSTED-SUPABASE.md). Follow the [mirror workflow](AWS-POSTGRES-IMAGE-MIRROR.md), then add narrow image, secret, and S3 paths. This host root has none of them.
