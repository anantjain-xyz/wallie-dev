...
| Policy           | Grant                                                                                                                                                                                                         |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Identity/network | Named host role with the rendered SSM and exact-repository ECR permissions boundary. Reviewed VPC, `database-a` subnet, Supabase database group, SSM endpoint group/rules, and `ssm`/`ssmmessages` endpoints. |
...
3. Render the host boundary with the reviewed account and region. It preserves the five SSM agent actions, caps ECR pulls at the exact owned `wallie-staging/supabase-postgres` repository, and caps future shell transcripts at `/wallie/staging/postgres/session`. The boundary alone does not grant transcript writes; the host root adds its inline policy only when [session logging](AWS-POSTGRES-SESSION-LOGGING.md) is enabled. An administrator creates `WallieStagingPostgresHostBoundary` from that document before a first host plan, or records its existing default version/document and makes this the new default after reviewing the diff. Preserve the previous default for rollback; stop if the five-version limit prevents that. Read back the document and ARN `arn:aws:iam::<account-id>:policy/WallieStagingPostgresHostBoundary`. Keep it as the host role's permanent boundary; the temporary deployment grant must not edit or remove it.

   ```sh
   umask 077
   mkdir -p .wallie/aws
   node scripts/prepare-aws-postgres-deployment.mjs \
     --policy host-boundary --account-id '<reviewed-12-digit-account>' \
     --region us-west-2 \
     > .wallie/aws/postgres-host-boundary-policy.json
   ```
...
4. Apply only the reviewed saved plan. Read back the instance, volumes, attachment, actual EBS KMS key, all security-group rules, endpoints and private DNS, instance role/profile and rendered boundary, and SSM registration. Require a zero-diff full plan. The data volume stays unformatted; do not start a database.
...
The host foundation may be applied before the [private image-pull path](AWS-POSTGRES-IMAGE-PULL.md). The current Supabase network grant does not authorize that later endpoint/route update; review a separate grant and plan before enabling it. No image pull or database runtime is qualified by this host apply.
