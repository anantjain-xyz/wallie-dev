...
- **Request magic link (Auth up).** Choose `Send magic link`. Run `node .cursor/skills/verify-wallie/scripts/control-wallie.mjs browser click --role button --name "Send magic link" --wait-for-text "Six-digit verification code"`. When Auth succeeds, capture that check-email / code state immediately: `... browser screenshot --path login-request.png` and `... browser snapshot --aria --path login-request.aria.txt`. When Auth is down, treat submit failure as a skip for `login-request` and still prove `landing-hero` / `landing-cta` / `login-form`.
