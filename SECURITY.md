...
- **Treat workspace-supplied content as untrusted** (e.g. Linear issue text, the session prompt) when it flows into a model prompt. The production prompt path is typed and compiler-enforced: `runStage()` classifies control text with `trustedPromptValue()` and session, feedback, and artifact data with `untrustedPromptValue()` before calling `renderStagePrompt()`, which accepts only those pre-classified values. Crossing into a renderer string happens only through `verifyPromptBoundary()` in `src/lib/pipeline/prompt-safety.ts`. The compile fixture in `src/lib/prompt-templates/index.typecheck.ts` uses `@ts-expect-error` to prove raw strings are rejected. When extending prompts, classify every new slot the same way; never pass a raw string across the boundary.
