...
- The [registry root](AWS-STAGING-REGISTRY.md) reserves an empty repository for the [pinned Supabase database image](SELF-HOSTED-SUPABASE.md). Mirror and verify that digest in a later batch, then add narrow image, secret, and S3 paths. This host root has none of them.
