...
create temp table publishing_session as
select *
from public.create_session_with_first_job(
  'b1b2c3d4-0001-4000-8000-000000000001',
  'c1b2c3d4-0001-4000-8000-000000000001',
  'Reject closes publishing generation',
  'Awaiting review, run still running.',
  'codex',
  'gpt-5.5',
  null,
  null,
...
from public.reject_session_stage(
  ...
  'b1b2c3d4-0001-4000-8000-000000000001',
  1,
  'retry after overlapping the publisher',
  'codex',
  'gpt-5.5',
  'project',
  'c1b2c3d4-0001-4000-8000-000000000001'
...
select is(
  (select status::text from public.agent_runs where id = (select run_id from publishing_session)),
  'canceled',
  'the publishing run is canceled so the original worker cannot mark it success'
);
select is(
  (select status::text from public.agent_jobs where id = (select job_id from publishing_session)),
  'success',
  'the publishing job is closed as success so the dedupe index releases'
);
select ok((select job_created from publishing_reject), 'a new rerun job is created instead of adopting the publisher');
select ok(
  (select job_id from publishing_reject) is distinct from (select job_id from publishing_session),
  'the queued rerun is not the publishing job'
);
select ok(
  exists (
    select 1
    from public.agent_runs run
    join publishing_reject result on result.run_id = run.id
    where run.agent_job_id = result.job_id
      and run.status = 'queued'
  ),
  'the replacement job has a queued run'
);
...
where id in (select session_id from phase_session)
   ...
   or id in (select session_id from publishing_session);
