...
- **Search.** Type a known seeded title fragment. Run `node .cursor/skills/verify-wallie/scripts/control-wallie.mjs browser fill --role searchbox --name "Search prompts, titles, or Linear IDs" --value "plan"`. Matching rows remain; unrelated titles drop out after results settle.
...
- Search debounces; wait for the list to update before asserting.
