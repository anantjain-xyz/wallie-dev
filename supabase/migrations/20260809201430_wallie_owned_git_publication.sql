...
set
  ...
  prompt_template_md = replace(
    replace(
      ...
      '- **Sweep PR feedback.** Gather every actionable item from top-level PR comments, inline review comments, and review states. Resolve each — a code change or an explicit, justified pushback. Loop until none remain and no required checks are failing (pending human-gated checks are fine; do not wait on them).',
      '- **Sweep existing PR feedback.** If an open PR already exists for this branch (for example, on a retry), gather every actionable item from top-level PR comments, inline review comments, and review states. Resolve each — a code change or an explicit, justified pushback — and loop until none remain and no required checks are failing (pending human-gated checks are fine; do not wait on them). If no PR exists yet, skip this sweep; Wallie creates it after this run.'
    ),
    '- **Verify against the plan.** Confirm every acceptance-criteria and validation item is met and CI is green on the latest commit; call out any gap. Close with a short verification summary so the human approver can sign off.',
    '- **Verify against the plan.** Confirm every acceptance-criteria and validation item is met and run all required local validation. If an open PR already exists, also confirm its required CI checks are green on the latest commit. If no PR exists yet, report the local validation evidence; Wallie''s PR creation after this run will trigger CI. Call out any gap and close with a short verification summary so the human approver can sign off.'
  )
...
as $$
  values
    ...
    (
      ...
      '- **Sweep existing PR feedback.** If an open PR already exists for this branch (for example, on a retry), gather every actionable item from top-level PR comments, inline review comments, and review states. Resolve each — a code change or an explicit, justified pushback — and loop until none remain and no required checks are failing (pending human-gated checks are fine; do not wait on them). If no PR exists yet, skip this sweep; Wallie creates it after this run.' || E'\n' ||
      '- **Verify against the plan.** Confirm every acceptance-criteria and validation item is met and run all required local validation. If an open PR already exists, also confirm its required CI checks are green on the latest commit. If no PR exists yet, report the local validation evidence; Wallie''s PR creation after this run will trigger CI. Call out any gap and close with a short verification summary so the human approver can sign off.' || E'\n'
    ),
    (
      3,
      'land',
...
-- Publication-only retries store their checkpoint in agent_jobs.last_error.
-- Keep that value while claiming the queued job so the processor can resume
-- PR bookkeeping instead of starting another coding-agent run. Successful
-- completion still clears last_error in markPipelineJobSuccess().
