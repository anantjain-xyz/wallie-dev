...
const usage =
  "Usage: node scripts/prepare-aws-registry.mjs policy|signing-policy|variables --account-id <12 digits> --region <region>";
...
function main() {
  ...
  if (
    ...
    !["policy", "signing-policy", "variables"].includes(command) ||
  ...
  const template = readFileSync(
    new URL(
      command === "signing-policy"
        ? "../infra/aws/signing-profile-policy.template.json"
        : "../infra/aws/registry-policy.template.json",
      import.meta.url,
    ),
    "utf8",
  );
